...
public class ResponseBuilder {
	...
	public CamResponse buildResponse(final String responseString) {
		...
		for(String row : rows) {
			...
			if(rowData.containsKey(ERROR_KEY) && !rowData.get(ERROR_KEY).equals(ZERO)) {
				error = true;
				...
				if(!rowData.containsKey(ERROR_KEY) && !rowData.containsKey(COUNT_KEY)) {
					data.add(rowData);
				}
			}
		}
		CamResponse response = new CamResponse(responseString, data, error, errorText);
		return response;
	}
}
